...
type ValidationType<T> = T extends Joi.BaseSchema<any>
  ? InterfaceFrom<NonNullable<T>>
  : any;
...
const makeValidator = <T extends ValidationSchemas>(
  schemas: T,
): ValidationHelpers<typeof schemas> => {
  const createValidator = (key: keyof ValidationSchemas) => (req: Request) => {
    if (!schemas[key]) {
      return req[key];
    }

    const { value, error } = (schemas[key] as Joi.BaseSchema<any>).validate(
      req[key],
    );
