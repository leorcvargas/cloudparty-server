...
const makeApp = ({
  ...
  const start = memo(async () => {
    if (appState !== AppState.IDLE)
      throw new Error('The application has already started.');
  ...
  const shutdown = (code: number) => async () => {
    ...
    if (
      (appState === AppState.STOPPING || appState === AppState.STOPPED) &&
      code === 0
    ) {
      if (forceShutdown) {
      ...
      logger.warn(
        'The application is yet to finishing the shutdown process. Repeat the command to force exit',
      );
      forceShutdown = true;
      return;
    }
  ...
  const lifecycleHooks = (
    decorator: (
      lifecycle: Lifecycle,
      fn: HookFn | HookFn[],
    ) => HookFn | HookFn[] = (lifecycle, fn) => fn,
  ) => {
    const once = (lifecycle, fn, order = 'append') => {
      const decoratedFn = decorator(lifecycle, fn);
      Array.isArray(decoratedFn)
        ? hooks[order](lifecycle, ...decoratedFn)
        : hooks[order](lifecycle, decoratedFn);
    };
...
const makeHookStore = (): HookStore => {
  ...
  const append = (lifecycle: Lifecycle, ...fn: HookFn[]) =>
    hooks.set(lifecycle, [...get(lifecycle), ...fn]);

  const prepend = (lifecycle: Lifecycle, ...fn: HookFn[]) =>
    hooks.set(lifecycle, [...fn, ...get(lifecycle)]);
